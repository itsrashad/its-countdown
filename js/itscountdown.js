...
/**
  * Plugin Name: its Countdown JS
  * Description: its Countdown JS
  * Version: 1.5
...
(function($) {
	...
	// global container for its countdown objects
	...
	var scd_counter = {
		...
		updateCounter : function(new_values, mode_changed) {
			...
			if(mode_changed === true) {
				...
				$('#' + this.options.id + ' .its-digit').remove();
		...
		updateCounterUnit : function(asset, old_value, new_value, initDisplay) {
			...
			var wrapper = $('#' + this.options.id + '-' + asset + '-digits'), count = wrapper.children('.its-digit').length;
			...
			for(i = 0; i < values['next'].length; i++) {
				...
				if(values['prev'][i] != values['next'][i] || initDisplay) {
					...
					this.updateCounterDigit(asset, $(wrapper).children('.its-digit').eq(i), values['next'].length - i - 1, values['prev'][i], values['next'][i], old_value, new_value, initDisplay);
		...
		updateDigitsCount : function(asset, current_count, old_value, new_value, wrapper) {
			...
			if(new_value.length < current_count) {
				...
				for(i = current_count - 1; i >= new_value.length; i--) {
					...
					wrapper.children('.its-digit').first().remove();
				...
				for(i = 0; i < digits_to_add.length; i++) {
					...
					new_digit = $('<div class="its-digit"></div>');
		...
		getLabel : function(asset, value) {
			...
			var suffix = itscountdown_plural(value);

			return itscountdownstrings[asset + suffix] || labels[asset];
		...
		setCounterUnitsVisibility : function(new_values) {
			...
			if(this.options.mode == 'up') {
				...
				if(this.options.hide_countup_counter == 1) {
					...
					counter_container.find('.its-counter').hide();

					counter_container.show();

				} else {

					counter_container.find('.its-counter').show();
		...
		setRowVerticalAlign : function() {
			...
			var digits = $('#' + this.options.id + ' .its-unit-vert .its-digits-row:visible');
			...
			var labels = $('#' + this.options.id + ' .its-unit-vert .its-label-row:visible');
			...
			labels = $('#' + this.options.id + ' .its-unit-horz .its-label:visible');
			...
			if(labels.length > 0) {
				...
				labels.each(function() {
					...
					if($this.hasClass('its-label-col')) {
			...
			if(this.options.hide_countup_counter != 1 || this.options.mode != 'up') {
				...
				$('#' + this.options.id).find('.its-counter-col').css('display', 'inline-block');
		...
		updateMinLabelsWidth : function() {
			...
			var test_label = $('#' + this.options.id + ' .its-label:visible').first(), unit, label_id, width, max_width = 0;
			...
			for(label_key in itscountdownstrings) {
				...
				test_label.text(itscountdownstrings[label_key]);
		...
		setLabelsPosition : function() {
			...
			$('#' + this.options.id + ' .its-label-col:visible').css('position', '');

			

			// adjust labels position if neeed (if vertical label position is set)

			var labels = $('#' + this.options.id + ' .its-label-row:visible');
			...
			if(labels.length > 0) {
				...
				labels.each(function() {
					...
					digitsDiv = $this.siblings('.its-digits-row');
		...
		resetAlignments : function(counter_container) {
			...
			counter_container.find('.its-label').css({ position : '', top : '' });

			counter_container.find('.its-label, .its-digits').css('min-width', '');
		...
		responsiveAdjust : function(/*width*/) {
			...
			if(responsive && responsive.length > 0) {
				...
				var is_wrapping = this.checkWrapping(counter_container.find('.its-unit-horz:visible, .its-title-row'));
				...
				var digit_groups = counter_container.find('.its-digits:visible'), is_wrapped = false;

				digit_groups.each(function() {

					var $this = $(this);

					var is_wrapping = self.checkWrapping($this.find('.its-digit'));
		...
		queryNextEvent : function(isNew) {
			...
			$('#' + self.options.id + ' .its-all-wrapper').hide();

			

			var queryData = {

				action : 'its_query_next_event',

				itscountdown_nonce : itscountdownajax.nonce,
			...
			$.getJSON(
					...
					function(response) {
						...
						$('#' + self.options.id + ' .its-all-wrapper').show();
