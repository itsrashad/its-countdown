<?php
/**
  * Plugin Name: its Countdown
  * Text Domain: its-countdown
  * Domain Path: /languages
  * Plugin URI: http://itsrashad.info
  * Description: Display and configure multiple its Countdown animated timers using a shortcode or sidebar widget.
  * Version: 1.5.2
...
class itsCountdown_Widget extends WP_Widget
{
	private static $defaults = array (
		...
		'digits_style' => '',
		'labels_style' => '',
		'base_font_size' => ITS_BASE_FONT_SIZE,
	...
	{
		$widget_ops = array (
			'classname' => 'widget_itscountdown',
			'description' => __ ('its Responsive Countdown Timer Widget.')
			);

		parent::__construct('itscountdown', __('its Countdown R'), $widget_ops);
		...
		add_action('wp_ajax_nopriv_scd_query_next_event', 'itsCountdown_Widget::queryNextEvent');
		add_action('wp_ajax_scd_query_next_event', 'itsCountdown_Widget::queryNextEvent');

		load_plugin_textdomain('its-countdown', false, dirname(plugin_basename(__FILE__)) . '/languages/');
	}

	public static function itscountdown_widget_init()
	{
		register_widget('itsCountdown_Widget');
	}
	...
	public function widget($args, $instance)
	{
		...
		$instance = itsCountdown_Helper::getCounterConfig($instance);
		...
		$instance['animations'] = itsCountdown_Helper::getAnimations($instance);
		...
		{
			// in customize preview other itscountdown widget may exist on the page.
			...
			if (!empty($customized))
			{
				$its_widgets_ids = array_keys($customized);
				...
				if (in_array($instance['id'], $its_widgets_ids))
				{
					$id = substr($instance['id'], strrpos($instance['id'], '-') + 1);

					// get new settings
					$new_instance = unserialize(base64_decode($customized['widget_itscountdown[' . $id . ']']['encoded_serialized_instance'], true));
		...
		// use apply_filters to sanitize / convert other widget output!
		...
		{
			?>
			<div id="<?php echo $instance['id']; ?>" style="font-size:<?php echo ITS_BASE_FONT_SIZE ?>px">
			<?php
		}
		...
		<div class="textwidget" <?php echo $widget_style; ?>><?php echo itsCountdown_Helper::getCounterHtml($instance); ?></div>
	...
	public function update($new_instance, $old_instance)
	{
		...
		if (!empty($new_instance['deadline']))
		{
			try
			{
				...
				$instance['deadline'] = $deadline->format('d-m-Y H:i:s');
			}
			catch (Exception $e)
			{
				$instance['deadline'] = '';
		...
		if ($instance['countup_limit'] == - 2 && itsCountdown_Helper::importPluginsEnabled())
		{
	...
	{
		// use apply_filters to sanitize / convert other widget output and form field values!
		$instance = wp_parse_args((array) $instance, array (
			'title' => 'its Countdown Timer',
			'title_before_down' => '',
			'title_after_down' => '',
			'title_before_up' => '',
			'title_after_up' => '',
		...
		$counter_modes = empty($instance['countdown_to_end']) || ! itsCountdown_Helper::importPluginsEnabled() ? (int) $instance['countdown_limit'] . ':' . (int) $instance['countup_limit'] : '-1:-2';
		...
		<p>
			<label><?php _e('Event date and time:', 'its-countdown'); ?></label>
			<input id="<?php echo $this->get_field_id('deadline'); ?>" name="<?php echo $this->get_field_name('deadline'); ?>" type="text" value="<?php echo $deadline; ?>" class="<?php echo $date_picker_class; ?>" />
		</p>
		<?php echo itsCountdown_Helper::enabledImportConfigs($this->get_field_id('import_config'), $this->get_field_name('import_config'), $import_config); ?>
		<p>
			<label for="<?php echo $this->get_field_id('counter_modes'); ?>"><?php _e('Counter display mode:', 'its-countdown'); ?></label>
			<?php $mode_options = array (
				'-1:-1' => __ ('Auto - both countdown and count up', 'its-countdown'),

				// '-1:60' => 'Quick up limit test',
				__('Only before event (countdown)', 'its-countdown') => array (
					'-1:0' => __('Countdown - no limit', 'its-countdown'),
					'3600:0' => sprintf(__('Show counter %s before event', 'its-countdown'), sprintf(_n('%d hour', '%d hours', 1, 'its-countdown'), 1)),
					'7200:0' => sprintf(__('Show counter %s before event', 'its-countdown'), sprintf(_n('%d hour', '%d hours', 2, 'its-countdown'), 2)),
					'86400:0' => sprintf(__('Show counter %s before event', 'its-countdown'), sprintf(_n('%d hour', '%d hours', 24, 'its-countdown'), 24)),
					'259200:0' => sprintf(__('Show counter %s before event', 'its-countdown'), sprintf(_n('%d day', '%d days', 3, 'its-countdown'), 3)),
					'604800:0' => sprintf(__('Show counter %s before event', 'its-countdown'), sprintf(_n('%d week', '%d weeks', 1, 'its-countdown'), 1))
					),
				__('Only after event(count up)', 'its-countdown') => array (
					'0:-1' => __('Count up - no limit', 'its-countdown'),
					'0:60' => sprintf(__('Hide counter %s after event', 'its-countdown'), sprintf(_n('%d minute', '%d minutes', 1, 'its-countdown'), 1)),
					'0:3600' => sprintf(__('Hide counter %s after event', 'its-countdown'), sprintf(_n('%d hour', '%d hours', 1, 'its-countdown'), 1)),
					'0:86400' => sprintf(__('Hide counter %s after event', 'its-countdown'), sprintf(_n('%d hour', '%d hours', 24, 'its-countdown'), 24)),
					'0:604800' => sprintf(__('Hide counter %s after event', 'its-countdown'), sprintf(_n('%d week', '%d weeks', 1, 'its-countdown'), 1))
					)
				);

				// add "countdown-to-end" option if at least one event import plugin is enabled
				if (itsCountdown_Helper::importPluginsEnabled())
				{
					$mode_options['-1:-2'] = __('Auto + countdown to event end while in progress', 'its-countdown');
				}

				echo itsCountdown_Helper::selectInput($this->get_field_id('counter_modes'), $this->get_field_name('counter_modes'), $counter_modes, array (
					'type' => 'optgroups',
		...
		<p>
			<label for="<?php echo $this->get_field_id('title_before_down'); ?>"><?php _e('Title before counter for countdown mode', 'its-countdown'); ?></label>
			<textarea class="widefat" id="<?php echo $this->get_field_id('title_before_down'); ?>" name="<?php echo $this->get_field_name('title_before_down'); ?>"><?php echo $title_before_down; ?></textarea>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('title_after_down'); ?>"><?php _e('Title after counter for countdown mode', 'its-countdown'); ?></label>
			<textarea class="widefat" id="<?php echo $this->get_field_id('title_after_down'); ?>" name="<?php echo $this->get_field_name('title_after_down'); ?>"><?php echo $title_after_down; ?></textarea>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('title_before_up'); ?>"><?php _e('Title before counter for count up mode', 'its-countdown'); ?></label>
			<textarea class="widefat" id="<?php echo $this->get_field_id('title_before_up'); ?>" name="<?php echo $this->get_field_name('title_before_up'); ?>"><?php echo $title_before_up; ?></textarea>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('title_after_up'); ?>"><?php _e('Title after counter for count up mode', 'its-countdown'); ?></label>
			<textarea class="widefat" id="<?php echo $this->get_field_id('title_after_up'); ?>" name="<?php echo $this->get_field_name('title_after_up'); ?>"><?php echo $title_after_up; ?></textarea>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('fx_preset'); ?>"><?php _e('Counter animation profile:', 'its-countdown'); ?></label>
			
			<?php echo itsCountdown_Helper::selectInput($this->get_field_id('fx_preset'), $this->get_field_name('fx_preset'), $fx_preset, array (
				'type' => 'filelist',
				'extension' => 'xml',
				...
				'folder' => array (
					...
					dirname(__FILE__) . '/../its-countdown-animations'
					)
				)
			); ?>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('layout_preset'); ?>"><?php _e('Widget layout preset:', 'its-countdown'); ?></label>

			<?php echo itsCountdown_Helper::selectInput($this->get_field_id('layout_preset'), $this->get_field_name('layout_preset'), $layout_preset, array (
				'type' => 'filelist',
				'extension' => 'xml',
				...
				'folder' => array (
					...
					dirname(__FILE__) . '/../its-countdown-custom-layouts'
					)
				)
			); ?>
		</p>
		<p>
			<input class="checkbox" id="<?php echo $this->get_field_id('hide_countup_counter'); ?>" name="<?php echo $this->get_field_name('hide_countup_counter'); ?>" type="checkbox" <?php echo ($hide_countup_counter ? ' checked' : ''); ?> />
			<label for="<?php echo $this->get_field_id('hide_countup_counter'); ?>"><?php _e('Use titles for count up mode as "Time has arrived" message', 'its-countdown'); ?></label>
		</p>
		<?php echo itsCountdown_Helper::checkboxesInput($this, $instance['units'], array('legend' => __('Display counter units:', 'its-countdown'))); ?>
		<p>
			<label for="<?php echo $this->get_field_id('title_before_size'); ?>"><?php _e('Text before counter font size:', 'its-countdown'); ?></label>

			<?php echo itsCountdown_Helper::selectInput($this->get_field_id('title_before_size'), $this->get_field_name('title_before_size'), $title_before_size, array (
				'type' => 'integer',
				'start' => 12,
				'end' => 50,
				'step' => 2
		...
		<p>
			<label for="<?php echo $this->get_field_id('title_after_size'); ?>"><?php _e('Text after counter font size:', 'its-countdown'); ?></label>
			<?php echo itsCountdown_Helper::selectInput($this->get_field_id ('title_after_size'), $this->get_field_name('title_after_size'), $title_after_size, array (
				'type' => 'integer',
				'start' => 12,
				'end' => 50,
				'step' => 2
		...
		<p>
			<label for="<?php echo $this->get_field_id('digits_size'); ?>"><?php _e('Counter digits size:', 'its-countdown'); ?></label>
			<?php echo itsCountdown_Helper::selectInput($this->get_field_id('digits_size'), $this->get_field_name('digits_size'), $digits_size, array (
				'type' => 'integer',
				'start' => 10,
				'end' => 80,
				'step' => 2
		...
		<p>
			<label for="<?php echo $this->get_field_id('labels_size'); ?>"><?php _e('Counter labels size:', 'its-countdown'); ?></label>
			<?php echo itsCountdown_Helper::selectInput($this->get_field_id('labels_size'), $this->get_field_name('labels_size'), $labels_size, array (
				'type' => 'integer',
				'start' => 8,
				'end' => 40,
				'step' => 2
...
<p>
	<label for="<?php echo $this->get_field_id( 'click_url' ); ?>"><?php _e( 'Goto URL on widget click (leave this field empty to disable this feature):', 'its-countdown' ); ?></label>
	<input class="widefat"
...
<p>
	<label for="<?php echo $this->get_field_id( 'redirect_url' ); ?>"><?php _e( 'Redirect to URL on contdown zero (leave this field empty to disable automatic redirection):', 'its-countdown' ); ?></label>
	<input class="widefat"
...
<p>
	<label for="<?php echo $this->get_field_id( 'widget_style' ); ?>"><?php _e( 'Widget style - CSS rules separated and ended with a semicolon:', 'its-countdown' ); ?></label>
	<input class="widefat"
...
<p>
	<label for="<?php echo $this->get_field_id( 'title_before_style' ); ?>"><?php _e( 'Text before counter style - CSS rules separated and ended with a semicolon:', 'its-countdown' ); ?></label>
	<input class="widefat"
...
<p>
	<label for="<?php echo $this->get_field_id( 'title_after_style' ); ?>"><?php _e( 'Text after counter style - CSS rules separated and ended with a semicolon:', 'its-countdown' ); ?></label>
	<input class="widefat"
...
<p>
	<label for="<?php echo $this->get_field_id( 'digits_style' ); ?>"><?php _e( 'Counter digits style - CSS rules separated and ended with a semicolon:', 'its-countdown' ); ?></label>
	<input class="widefat"
...
<p>
	<label for="<?php echo $this->get_field_id( 'labels_style' ); ?>"><?php _e( 'Counter labels style - CSS rules separated and ended with a semicolon:', 'its-countdown' ); ?></label>
	<input class="widefat"
...
<?php
	...
	public static function admin_scripts($hook) {
		if ($hook == 'widgets.php') {
			...
			wp_register_script ( 'itscountdown-admin-script', $plugin_url . '/js/vendor/timepicker.collapse.js', array (
					'jquery',
					'jquery-ui-datepicker',
					'jquery-ui-slider',
					'jquery-ui-timepicker-addon' 
			) );
			wp_enqueue_script ( 'itscountdown-admin-script' );
	...
	public static function counter_scripts() {
		...
		*/
		...
		wp_register_script ( 'itscountdown-counter-script', $plugin_url . '/js/itscountdown.js', array (
				'jquery' 
		) );
		wp_enqueue_script ( 'itscountdown-counter-script' );
		
		wp_register_style ( 'itscountdown-counter-style', $plugin_url . '/css/itscountdown.css' );
		wp_enqueue_style ( 'itscountdown-counter-style' );
		
		if (is_rtl()) {
			// load rtl styles overloads
			wp_register_style ( 'itscountdown-counter-style-rtl', $plugin_url . '/css/itscountdown_rtl.css' );
			wp_enqueue_style ( 'itscountdown-counter-style-rtl' );
		}
		
		wp_localize_script ( 'itscountdown-counter-script', 'itscountdownajax', array (
				'url' => admin_url ( 'admin-ajax.php' ),
				'nonce' => wp_create_nonce ( 'its_query_next_event' ) 
		) );
		...
		wp_register_script ( 'itscountdown-plural-strings', $plural_js_file );
		wp_enqueue_script ( 'itscountdown-plural-strings' );
	...
	/**
	 * Query next event for its Countdown.
	 */
	public static function queryNextEvent() {
		...
		if (! check_ajax_referer ( 'its_query_next_event', 'itscountdown_nonce', false )) {
			...
			if (! empty ( $_REQUEST ['id'] )) {
				...
				if (strpos ( $id, 'itscountdown-' ) !== 0) {
					$response ['err_code'] = 101;
					$response ['err_msg'] = 'Invalid Request!';
				}
				
				$id = ( int ) substr ( $id, strlen ( 'itscountdown-' ) );
				
				$widgets = get_option ( 'widget_itscountdown' );
				...
				if (isset ( $widgets [$id] ) || ! empty ( $_REQUEST ['customize_preview'] )) {
					...
					if (! empty ( $instance ['import_config'] )) {
						...
						$instance = apply_filters ( 'itscountdownr_get_event', $instance, $now_ts );
						...
						if (isset ( $instance ['imported'] )) {
							...
							if (empty ( $instance ['imported'] )) {
								...
								$instance = itsCountdown_Helper::processImportedEvents ( $instance, $now_ts );
							}
						} else {
						...
						$instance = itsCountdown_Helper::updateDeadlineUTC ( $instance );
				...
				if (empty ( $import_config )) {
					try {
						...
						$deadlineUTC = itsCountdown_Helper::localDateToUTC ( $deadline );
					...
					$options = apply_filters ( 'itscountdownr_get_event', $options, $now_ts );
					if (isset ( $options ['imported'] )) {
						...
						if (empty ( $options ['imported'] )) {
							...
							$options = itsCountdown_Helper::processImportedEvents ( $options, $now_ts );
						}
					} else {
						// import plugins were desactivated or uninstalled
						$options ['deadline'] = '';
	...
	public static function short_code($atts, $content = null) {
		...
		$atts ['shortcode'] = 1;
		$tmp = new itsCountdown_Widget ( $atts ['id'], __ ( 'its Countdown R' ) );
...
add_action ( 'widgets_init', 'itsCountdown_Widget::itscountdown_widget_init' );
add_shortcode ( 'itscountdown', 'itsCountdown_Widget::short_code' );
function itscountdown_activation_check() {
	if (version_compare ( $GLOBALS ['wp_version'], SCD_MINIMUM_REQUIRED_WP_VERSION, '<' )) {
		load_plugin_textdomain ( 'its-countdown' );
		
		$message = '<strong>' . sprintf ( esc_html__ ( 'its Countdown R requires WordPress %s or higher.', 'its-countdown' ), SCD_MINIMUM_REQUIRED_WP_VERSION ) . '</strong>';
		echo $message;
		
		exit ();
	}
}
function itscountdown_uninstall() {
	delete_option ( 'widget_itscountdown' );
	delete_site_option ( 'widget_itscountdown' );
}
register_activation_hook ( __FILE__, 'itscountdown_activation_check' );
register_uninstall_hook ( __FILE__, 'itscountdown_uninstall' );
