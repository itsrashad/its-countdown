...
define ( 'ITS_BASE_FONT_SIZE', 12 );

abstract class itsCountdown_Helper
{
	...
	private static $layout_tpls = array (
		'labels_its' => array (
			'row' => array (
				array (
					'selector' => '.its-label',
					'remove' => 'its-label-col its-label-row',
					'add' => 'its-label-row'
					),
				array (
					'selector' => '.its-digits',
					'remove' => 'its-digits-col its-digits-row',
					'add' => 'its-digits-row'
					)
				),
			'col' => array (
		...
		'layout' => array (
			'vert' => array (
				'selector' => '.its-unit',
				'remove' => 'its-unit-vert its-unit-horz',
				'add' => 'its-unit-vert clearfix'
				),
			'horz' => array (
				'selector' => '.its-unit',
				'remove' => 'its-unit-vert its-unit-horz clearfix',
				'add' => 'its-unit-horz'
				)
			),
		'event_text_pos' => array (
			'vert' => array (
				array (
					'selector' => '.its-title',
					'remove' => 'its-title-col its-title-row clearfix',
					'add' => 'its-title-col clearfix'
					),
				array (
					'selector' => '.its-counter',
					'remove' => 'its-counter-col its-counter-row clearfix',
					'add' => 'its-counter-col clearfix'
					)
				),
			'horz' => array (
				array (
	...
	public static function getCounterConfig($instance)
	{
		if (!empty($instance['layout_preset']))
		{
			...
			if (!file_exists($file_name))
			{
				...
				$file_name = dirname ( __FILE__ ) . '/../../its-countdown-custom-layouts/' . $instance ['layout_preset'];
			}
			if (!file_exists($file_name))
			{
			...
			$instance ['labels_its'] = (string) $xml->labels_its;
			...
			if ($is_responsive)
			{
				...
				$labels_its = $instance['labels_its'] == 'right' || $instance['labels_its'] == 'left' ? 'row' : 'col';
				$classes[] = self::$layout_tpls['labels_its'][$labels_its];
			...
			$instance['base_font_size'] = ITS_BASE_FONT_SIZE;
		}
	...
	private static function getCounterLayout($instance)
	{
		...
		$title_before_size = $instance['title_before_size'] / ITS_BASE_FONT_SIZE;
		$title_after_size = $instance['title_after_size'] / ITS_BASE_FONT_SIZE;
		$labels_size = $instance['labels_size'] / ITS_BASE_FONT_SIZE;

		$layout['event_text_pos'] = $instance['event_text_pos'];
		$layout['labels_its'] = $instance['labels_its'];
		...
		switch ($layout['labels_its'])
		{
			case 'left' :
			case 'right' :
				$layout['labels_class'] = 'its-label its-label-row';
				$layout['digits_class'] = 'its-digits its-digits-row ';
				break;
			case 'top' :
			case 'bottom' :
			default :
				$layout['labels_class'] = 'its-label its-label-col';
				$layout['digits_class'] = 'its-digits its-digits-col';
				break;
		}
		...
		{
			case 'horz' :
				$layout['text_class'] = 'its-title its-title-row';
				$layout['counter_class'] = 'its-counter its-counter-row its-counter-' . $instance['layout'];
				break;
			case 'vert' :
			default :
				$layout['text_class'] = 'its-title its-title-col clearfix';
				$layout['counter_class'] = 'its-counter its-counter-col';
		}

		$layout['units_class'] = 'its-unit its-unit-' . $instance['layout'];
	...
	private static function sortEvents($timeline, $sort_external = 'asc') {
		...
		if (! function_exists ( 'its_padded_numeric_sort' )) {

			function its_padded_numeric_sort($a, $b) {
		...
		foreach ( $timeline as &$group ) {
			...
			uksort ( $group, 'its_padded_numeric_sort' );
	...
	public static function getCounterHtml($instance) {
		...
		$layout = itsCountdown_Helper::getCounterLayout ( $instance );
...
<div class="its-all-wrapper">
	<div class="<?php echo $layout['text_class']; ?> its-title-before"
	...
	<div class="<?php echo ($layout['counter_class']); ?>">
		...
		<?php foreach(self::$assets as $asset) : ?>
			...
			<?php if($instance['labels_its'] == 'left' || $instance['labels_its'] == 'top') : ?>
	...
	<div class="<?php echo $layout['text_class']; ?> its-title-after"
...
<?php
	...
	public static function getAnimations($instance) {
		...
		foreach ( $xml->digit as $digit ) {
			...
			// effective scaling (basing on ITS_BASE_FONT_SIZE)
			...
			foreach ( $digit->styles->style as $value ) {
				...
				if (($attrs ['name'] == 'height' || $attrs ['name'] == 'font-size') && ! empty ( $attrs ['scalable'] )) {
					...
					if ($attrs ['unit'] == 'px') {
						...
						$scale = ($digits_size / ITS_BASE_FONT_SIZE) / $attrs ['value'];
			...
			foreach ( $digit->styles->style as $value ) {
				...
				if ($attrs ['unit'] == 'px') {
					...
					$attrs ['value'] = $attrs ['value'] / ITS_BASE_FONT_SIZE;
	...
	public static function checkboxesInput($widget, $values, $config = array()) {
		...
		$lang_key_units = array (
				...
				'years' => __ ( 'years', 'its-countdown' ),

				'months' => __ ( 'months', 'its-countdown' ),

				'weeks' => __ ( 'weeks', 'its-countdown' ),

				'days' => __ ( 'days', 'its-countdown' ),

				'hours' => __ ( 'hours', 'its-countdown' ),

				'minutes' => __ ( 'minutes', 'its-countdown' ),

				'seconds' => __ ( 'seconds', 'its-countdown' ) 
	...
	public static function enabledImportConfigs($id, $name, $selected = '') {
		...
		$configs = apply_filters ( 'itscountdownr_get_import_configs', $configs );
		...
		$html [] = '<label for="' . $id . '">' . __ ( 'Import events from:', 'its-countdown' ) . '</label>';
		...
		$html [] = esc_html__ ( 'Disabled. Use event date and time from settings', 'its-countdown' );
		...
		foreach ( $configs as $provider => $presets ) {
			...
			foreach ( $presets as $v => $text ) {
				...
				$html [] = $text != '' ? esc_html ( $text ) : esc_html__ ( 'Invalid configuration', 'its-countdown' );
		...
		$html [] = __ ( 'Widget event date and time will be ignored if an event import configuration is selected', 'its-countdown' );
